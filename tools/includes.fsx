...
module private Cmd =
    ...
    let appendTo (file: string) (content: string) =
        log "Append" "%s" file
...
and NuGetDependencyVersion =
    ...
    member this.PaketDeps =
        match this with
        | LockVersion | CurrentVersion -> "staging"
        | NoVersion -> ""

    member this.PaketTemplate =
        match this with
...
and NuGetReference =
    ...
    member this.Latest(allowPreRelease) =
        { this with pre = allowPreRelease }
...
and BuildTool =
    ...
    member this.Dispatch (s: Solution) =
        ...
        |> List.choose (function
            ...
            | PaketDependency.NuGet (name, ver) ->
                sprintf "nuget %s %s" name ver.PaketDeps |> Some
...
and WebSharper4Projects(tool: BuildTool) =
    ...
    member this.HostWebsite name =
        { tool = tool
          file = sprintf "%s/%s.csproj" name name } : MSBuildProject
...
and MSBuildProject =
    ...
    member this.References(_: ReferenceBuilder -> #seq<Reference>) = this
...
and NuGetPackage =
    ...
    member this.Description(descr) =
        { this with config = { this.config with Description = descr } }
    ...
    interface IProject with
        member this.Write(hasNet4Version) =
            ...
            let deps =
                ...
                |> List.choose (function
                    | PaketDependency.NuGet (name, ver) ->
                        Some <| sprintf "    %s %s" name ver.PaketTemplate
